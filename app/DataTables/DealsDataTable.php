...
class DealsDataTable extends BaseDataTable
{
    ...
    public function dataTable($query)
    {
        ...
        $datatables->addColumn('export_added_by', fn($row) => $row->addedBy->name ?? '--');
        $datatables->addColumn('action', function ($row) {
            ...
            if (
                ...
                || ($this->editLeadPermission == 'both' && ((!is_null($row->agent_id) && user()->id == $row->leadAgent->user->id) || user()->id == $row->added_by))
            ) {
        ...
        $datatables->editColumn('agent_name', fn($row) => $row->agent_id ? view('components.employee-image', ['user' => $row->leadAgent->user]) : '--');
        $datatables->addColumn('added_by', fn($row) => $row->added_by ? view('components.employee', ['user' => $row->addedBy]) : '--');
    ...
    {
        $lead = $model->with(['leadAgent', 'leadAgent.user', 'category', 'contact', 'pipeline', 'leadStage'])
            ->select(
                'deals.id',
                'deals.name',
                'deals.lead_id',
    ...
    protected function getColumns()
    {
        ...
        $data = [
            ...
            __('app.addedBy') => ['data' => 'export_added_by', 'name' => 'users.name', 'exportable' => true, 'visible' => false, 'title' => __('app.addedBy')],
            __('app.deal') . ' ' . __('app.addedBy') => ['data' => 'added_by', 'name' => 'users.name', 'title' => __('app.deal') . ' ' . __('app.addedBy')],
